								...
								<div class="signin-form_box_sub" id="signin-form_box_sub_recover">
									...
									<form class="password-recover" action="/password-recover.php" method="post" {{if ( $xml->getItem("/shop/@sslurl")) and ( $xml->getItem("/shop/@sslurl") neq (string) '')}} action="{{$xml->getItemEscape("/shop/@sslurl")}}password-recover.php"{{/if}}>
										...
										<div class="signin_inputs" id="signin_inputs_recover">
											<label class="reminder_label_signin">{{$txt['67952::51100_007']}}
											</label>
											<input id="signin_pass_recover_input" type="text" name="email"/>
											<button {{assign "classAttributeTmp3" ""}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}signin_password-recover{{/capture}} type="submit"{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}btn-small signin_password-recover{{/capture}}  class="{{$classAttributeTmp3}}">{{$txt['67952::51100_003']}}
											</button>
										</div>
									</form>
									<div class="n67952_phone">
										<div class="n67952_phone_sub">
											<span class="n67952_infotext_label">{{$txt['67952::51100_004']}}
												<b>{{literal}}{{/literal}}{{$xml->getItemEscape("page/login/response/@tel")}}</b>{{literal}}{{/literal}}{{$txt['67952::51100_005']}}{{literal}}{{/literal}}
												<a class="mail_54144" href="mailto:{{$xml->getItemEscape("page/login/response/@email")}}">{{$xml->getItemEscape("page/login/response/@email")}}</a>.</span>
										</div>
									</div>
								</div>
							</div>
