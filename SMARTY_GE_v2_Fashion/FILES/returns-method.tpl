								...
								<div {{assign "classAttributeTmp3" ""}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}progress_bar_wrapper{{/capture}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}progress_bar_wrapper pb_step3{{/capture}}  class="{{$classAttributeTmp3}}">
									...
									<div class="_step _step1">
										<span>
											1</span>
										<p>
											<i></i>{{$txt['69372::step1_txt']}}
										</p>
									</div>
									<div class="_step _step2">
										<span>2</span>
										<p>
											<i></i>{{$txt['69372::step2_txt']}}
										</p>
									</div>
									<div class="_step _step3">
										<span>3</span>
										<p>
											<i></i>{{$txt['69372::step3_txt']}}
										</p>
									</div>
									<div class="_step _step4">
										<span>4</span>
										<p>
											<i></i>{{$txt['69372::step4_txt']}}
										</p>
									</div>
									<div class="_step _step5">
										<span>5</span>
										<p>
											<i></i>{{$txt['69372::step5_txt']}}
										</p>
									</div>
									<div class="_step _step6">
										<span>6</span>
										<p>
											<i></i>{{$txt['69372::step6_txt']}}
										</p>
									</div>
								</div>
								<h1 class="big_label return_label">{{$txt['69372::51900_00634']}}
								</h1>
								<form class="return_type">
									{{if count( $xml->getList("page/return_methods/return_method"))  gt (string) 1}}
										<div class="table_display">
											{{foreach from=$xml->getList("page/return_methods/return_method") item=loop360}}
												<label {{assign "classAttributeTmp4" ""}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}return_type{{/capture}}{{if $loop360->getItem("@checked") == (string) 'true'}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}return_type active{{/capture}}{{/if}}  class="{{$classAttributeTmp4}}">
													...
													{{if $n69372_icon_txt and  $loop360->getItem("@icon")}}
														<img class="n69372_icon" src="{{$loop360->getItemEscape("@icon")}}" alt="{{$loop360->getItemEscape("@name")}}"></img>
													{{/if}}
													<div class="return_name">
														{{if  $loop360->getItem("@id")  == (string) 1}}
															{{$txt['69372::51900_1']}}
														{{elseif  $loop360->getItem("@id")  == (string) 2}}
															{{$txt['69372::51900_2']}}
														{{elseif  $loop360->getItem("@id")  == (string) 3}}
															{{$txt['69372::51900_3']}}
														{{elseif  $loop360->getItem("@name")}}
													...
													<div class="return_desc">
														...
														{{elseif  $loop360->getItem("@id")  == (string) 3}}
															{{$txt['69372::51900_3_d']}}
														{{elseif  $loop360->getItem("@description")}}
															{{$loop360->getItemEscape("@description")}}
														{{/if}}
													</div>
													{{if $n69372_shipping_title and   $loop360->getItem("/shop/page/return_methods/@shipping_balanced_refund")  == (string) 'true' and  $loop360->getItem("@id")  == (string) 2}}
														<div class="n69372_shipping_balance">
															<strong>{{$txt['69372::n69372_shipping_title']}}</strong>
															<div>{{$txt['69372::n69372_shipping_balance']}}
															</div>
														</div>
													{{/if}}
													{{if $n69372_p_txt and   $loop360->getItem("shipping/delivery")}}
														<div class="return_methods_label">
															<span class="return_methods_labe1">{{$n69372_p_txt}}</span>
															<span class="return_methods_labe2">{{$txt['69372::51900_3_koszt']}}</span>
														</div>
														{{foreach from=$loop360->getList("shipping/delivery") item=loop412}}
														...
														{{/foreach}}
														<div class="return_methods_info">{{$txt['69372::51900_3_forma']}}
														</div>
													{{/if}}
												</label>
											{{/foreach}}
									...
									{{else}}
										<div class="return_type_one">
											{{foreach from=$xml->getList("page/return_methods/return_method") item=loop436}}
												<input class="return_type" name="return_type" type="radio" value="{{$loop436->getItemEscape("@id")}}" checked="checked"></input>
												<div class="return_one_name">
													{{if  $loop436->getItem("@id")  == (string) 1}}
														{{$txt['69372::51900_1']}}
													{{elseif  $loop436->getItem("@id")  == (string) 2}}
														{{$txt['69372::51900_2']}}
													{{elseif  $loop436->getItem("@id")  == (string) 3}}
														{{$txt['69372::51900_3']}}
													{{elseif  $loop436->getItem("@name")}}
														{{$loop436->getItemEscape("@name")}}
													{{/if}}
												</div>
												<div class="return_one_desc">
													...
													{{elseif  $loop436->getItem("@id")  == (string) 3}}
														{{$txt['69372::51900_3_d']}}
													{{elseif  $loop436->getItem("@description")}}
														{{$loop436->getItemEscape("@description")}}
													{{/if}}
												</div>
											{{/foreach}}
										</div>
									...
									<div class="returns_summary_buttons table_display">
										<div class="btn_wrapper backlink">
											{{if  $xml->getItem("page/backlink/@url")}}
												<a class="btn" href="{{$xml->getItemEscape("page/backlink/@url")}}">{{$txt['69372::order2_button_back']}}</a>
											{{/if}}
										</div>
										<div class="btn_wrapper">
											<button id="return_go_next" type="submit" class="n53706_button1 btn-large btn_arrow">{{$txt['69372::50001_10a']}}
											</button>
										</div>
									</div>
								</form>
