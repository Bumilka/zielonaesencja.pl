			...
			<div id="layout" class="{{$txt['73991::70925_layout_class']}}">
				...
				{{if !( $xml->getItem("action/set_render/item")) or ( $xml->getItem("action/set_render/item/@name")  == (string) 'content')}}
					<div {{assign "classAttributeTmp2" ""}}  id="content"{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}span9{{/capture}}{{if $xml->getItem("page[@type='projector']")}} itemscope="itemscope" itemtype="http://schema.org/Product" {{/if}}{{if $xml->getItem("page/@type") == (string) 'projector'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['73991::70925_projector_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'search'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['73991::70925_search_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'main'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['73991::70925_main_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'navigation'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['73991::70925_navigation_page_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'blog-list' or $xml->getItem("page/@type") == (string) 'blog-item'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['73991::70925_blog_c_class']}}{{/capture}}{{else}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['73991::70925_otherwise_c_class']}}{{/capture}}{{/if}}  class="{{$classAttributeTmp2}}">
						...
						<!--Lista kategorii (categories, 81124.1)-->
						<h1 class="categories-list_label big_label">{{$txt['81124::30003_01']}}
						</h1>
						<div class="categories-list_wrapper">
							{{if  $xml->getItem("page/categories-list/@mode") == (string)'2'}}
								<ul class="categories-list_level1">
									{{foreach from=$xml->getList("/shop/navigation2/item") item=loop243}}
										<li class="categories-list_level1">
											{{if  $loop243->getItem("@link")}}
												<a class="categories-list_level1" href="{{$loop243->getItemEscape("@link")}}" title="{{$loop243->getItemEscape("@name")}}">{{$loop243->getItem("@name")}}</a>
											{{else}}
												<span class="categories-list_level1">{{$loop243->getItem("@name")}}</span>
											{{/if}}
											{{if  $loop243->getItem("item")}}
												<span {{assign "classAttributeTmp3" ""}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}more_categories{{/capture}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}more_categories visible-phone{{/capture}}  class="{{$classAttributeTmp3}}">{{$txt['81124::40002_000']}}</span>
												<ul class="categories-list_level2">
													{{foreach from=$loop243->getList("item") item=loop263}}
														<li class="categories-list_level2">
															{{if  $loop263->getItem("@link")}}
																<a class="categories-list_level2" href="{{$loop263->getItemEscape("@link")}}" title="{{$loop263->getItemEscape("@name")}}">{{$loop263->getItem("@name")}}</a>
															{{else}}
																<span class="categories-list_level2">{{$loop263->getItem("@name")}}</span>
															{{/if}}
															{{if  $loop263->getItem("item")}}
																<span {{assign "classAttributeTmp3" ""}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}more_categories{{/capture}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}more_categories visible-phone{{/capture}}  class="{{$classAttributeTmp3}}">{{$txt['81124::40002_000']}}</span>
																<ul class="categories-list_level3">
																	{{foreach from=$loop263->getList("item") item=loop283}}
																		<li class="categories-list_level3">
																			{{if  $loop283->getItem("@link")}}
																				<a class="categories-list_level3" href="{{$loop283->getItemEscape("@link")}}" title="{{$loop283->getItemEscape("@name")}}">{{$loop283->getItem("@name")}}</a>
																			{{else}}
																				<span class="categories-list_level3">{{$loop283->getItem("@name")}}</span>
																			{{/if}}
																		</li>
																	{{/foreach}}
																</ul>
							...
							{{else}}
								<ul class="categories-list_level1">
									{{foreach from=$xml->getList("/shop/navigation/item") item=loop313}}
										<li class="categories-list_level1">
											{{if  $loop313->getItem("@link")}}
												<a class="categories-list_level1" href="{{$loop313->getItemEscape("@link")}}" title="{{$loop313->getItemEscape("@name")}}">{{$loop313->getItem("@name")}}</a>
											{{else}}
												<span class="categories-list_level1">{{$loop313->getItem("@name")}}</span>
											{{/if}}
											{{if  $loop313->getItem("item")}}
												<span {{assign "classAttributeTmp3" ""}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}more_categories{{/capture}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}more_categories visible-phone{{/capture}}  class="{{$classAttributeTmp3}}">{{$txt['81124::40002_000']}}</span>
												<ul class="categories-list_level2">
													{{foreach from=$loop313->getList("item") item=loop333}}
														<li class="categories-list_level2">
															{{if  $loop333->getItem("@link")}}
																<a class="categories-list_level2" href="{{$loop333->getItemEscape("@link")}}" title="{{$loop333->getItemEscape("@name")}}">{{$loop333->getItem("@name")}}</a>
															{{else}}
																<span class="categories-list_level2">{{$loop333->getItem("@name")}}</span>
															{{/if}}
															{{if  $loop333->getItem("item")}}
																<span {{assign "classAttributeTmp3" ""}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}more_categories{{/capture}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}more_categories visible-phone{{/capture}}  class="{{$classAttributeTmp3}}">{{$txt['81124::40002_000']}}</span>
																<ul class="categories-list_level3">
																	{{foreach from=$loop333->getList("item") item=loop353}}
																		<li class="categories-list_level3">
																			{{if  $loop353->getItem("@link")}}
																				<a class="categories-list_level3" href="{{$loop353->getItemEscape("@link")}}" title="{{$loop353->getItemEscape("@name")}}">{{$loop353->getItem("@name")}}</a>
																			{{else}}
																				<span class="categories-list_level3">{{$loop353->getItem("@name")}}</span>
																			{{/if}}
																		</li>
																	{{/foreach}}
																</ul>
