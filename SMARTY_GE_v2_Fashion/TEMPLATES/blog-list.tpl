			...
			<div id="layout" class="{{$txt['73991::70925_layout_class']}}">
				...
				{{if !( $xml->getItem("action/set_render/item")) or ( $xml->getItem("action/set_render/item/@name")  == (string) 'content')}}
					<div {{assign "classAttributeTmp2" ""}}  id="content"{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}span9{{/capture}}{{if $xml->getItem("page[@type='projector']")}} itemscope="itemscope" itemtype="http://schema.org/Product" {{/if}}{{if $xml->getItem("page/@type") == (string) 'projector'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['73991::70925_projector_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'search'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['73991::70925_search_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'main'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['73991::70925_main_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'navigation'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['73991::70925_navigation_page_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'blog-list' or $xml->getItem("page/@type") == (string) 'blog-item'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['73991::70925_blog_c_class']}}{{/capture}}{{else}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['73991::70925_otherwise_c_class']}}{{/capture}}{{/if}}  class="{{$classAttributeTmp2}}">
						...
						<div class="blog_categoriesdescription mrg-b">
							<h1 class="big_label">
							...
							{{if !( $xml->getItem("page/blogitems/additional/category_list/description")  == (string) '') and ( $xml->getItem("page/blogitems/additional/category_list/description")) }}
								{{if (( $xml->getItem("/shop/page/blogitems/navigation/item[1]/@counter")  == (string)1) and ( $xml->getItem("/shop/page/blogitems/navigation/item[1]/@type")  == (string)'current'))}}
									<div class="blog_categoriesdescription_sub clear">{{$xml->getItem("page/blogitems/additional/category_list/description")}}
									</div>
								{{/if}}
							{{/if}}
						</div>
						...
						{{if count( $xml->getList("/shop/page/blogitems/blogitem"))  gt (string) 0}}
							{{capture name="blog_list_blazy_off" assign="blog_list_blazy_off"}}
								{{$txt['81758::blog_list_blazy_off']}}
							{{/capture}}
							<div id="blog_list">
								<div class="row clearfix">
									{{foreach from=$xml->getList("/shop/page/blogitems/blogitem") item=loop328}}
										<div {{assign "classAttributeTmp3" ""}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}blog_list_wrapper{{/capture}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}blog_list_wrapper {{$txt['81758::53698_class']}}{{/capture}}  class="{{$classAttributeTmp3}}">
											{{if  $loop328->getItem("more/@href") and !( $loop328->getItem("more/@href") == (string)'')}}
												<h2>
													<a class="blog_name" href="{{$loop328->getItemEscape("more/@href")}}" title="{{$loop328->getItem("title")}}">{{$loop328->getItem("title")}}</a>
												</h2>
											{{else}}
												{{if  $loop328->getItem("title")  neq (string) ''}}
													<h2>
														<span class="blog_name">{{$loop328->getItem("title")}}</span>
													</h2>
												{{/if}}
											{{/if}}
											<div class="blog_list_item description clearfix">
												{{if  $loop328->getItem("image/@src") and  $loop328->getItem("image/@src") neq (string)''}}
													{{if  $loop328->getItem("@link")}}
														{{literal}}
														<a class="blog_image" href="{{/literal}}{{$loop328->getItemEscape("@link")}}{{literal}}" {{/literal}}{{else}}{{literal}}
<span class="blog_image" {{/literal}}{{/if}}{{literal}}>{{/literal}}
															{{if $blog_list_blazy_off  neq (string)''}}
																<img src="{{$loop328->getItem("image/@src")}}" class="blog_image" alt="{{$loop328->getItem("title/text()")}}"></img>
															{{else}}
																<img src="/gfx/__IAI_TEMPLATE_LANG_639_1__/loader.gif?r=1570606529" data-src-small="{{$loop328->getItem("image/@src")}}" data-src="{{$loop328->getItem("image/@src")}}" class="blog_image b-lazy" alt="{{$loop328->getItem("title/text()")}}"></img>
															{{/if}}
															{{if  $loop328->getItem("@link")}}
																{{literal}}</a>{{/literal}}
														{{else}}
													...
													{{/if}}
												{{/if}}
												{{$loop328->getItem("description")}}
												<br/>
												{{if  $loop328->getItem("more/@href") and !( $loop328->getItem("more/@href") == (string)'')}}
													<a {{assign "classAttributeTmp4" ""}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}see_more{{/capture}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}see_more btn-regular blog{{/capture}} href="{{$loop328->getItemEscape("more/@href")}}" title="{{$loop328->getItem("title")}}"  class="{{$classAttributeTmp4}}">{{$txt['81758::53698_xx']}}</a>
												{{/if}}
											</div>
										</div>
									{{/foreach}}
						...
						{{/if}}
						<!--Blog - Paging (blog_list_paging, 81760.1)-->
						<div class="paging_setting" id="blog_paging">
							{{if  $xml->getItem("/shop/page/blogitems/navigation/@total")  gt (string)  $xml->getItem("/shop/page/blogitems/navigation/@portions")}}
								<div class="search_paging">
									<div class="search_paging_sub">
										{{if  $xml->getItem("/shop/page/blogitems/navigation/prev")}}
											<a class="previous" href="{{$xml->getItem("/shop/page/blogitems/navigation/path/@value")}}{{$xml->getItemEscape("/shop/page/blogitems/navigation/prev/@counter")}}">{{$txt['81760::40011_001']}}</a>
										{{/if}}
										{{if !( $xml->getItem("/shop/page/blogitems/navigation/prev"))}}
											<span class="previous">{{$txt['81760::40011_001']}}</span>
										{{/if}}
										{{foreach from=$xml->getList("/shop/page/blogitems/navigation/item") item=loop410}}
											{{if  $loop410->getItem("@gap")}}
												<span class="gap">{{$txt['81760::40011_123']}}</span>
											{{/if}}
											{{if !( $loop410->getItem("@type")  == (string) 'current') and !( $loop410->getItem("@gap"))}}
												<a class="paging" href="{{$loop410->getItem("../path/@value")}}{{$loop410->getItemEscape("@counter") - 1}}">{{$loop410->getItemEscape("@counter")}}</a>
											{{/if}}
											{{if  $loop410->getItem("@type")  == (string) 'current'}}
												<span class="current">{{$loop410->getItemEscape("@counter")}}</span>
											{{/if}}
										{{/foreach}}
										{{if  $xml->getItem("/shop/page/blogitems/navigation/next")}}
											<a class="next" href="{{$xml->getItem("/shop/page/blogitems/navigation/path/@value")}}{{$xml->getItemEscape("/shop/page/blogitems/navigation/next/@counter")}}">{{$txt['81760::40011_005']}}</a>
										{{/if}}
										{{if !( $xml->getItem("/shop/page/blogitems/navigation/next"))}}
											<span class="next">{{$txt['81760::40011_005']}}</span>
										{{/if}}
									</div>
								</div>
							{{/if}}
