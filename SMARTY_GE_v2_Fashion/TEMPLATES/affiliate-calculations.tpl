					...
					<div {{assign "classAttributeTmp2" ""}}  id="content"{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}span9{{/capture}}{{if $xml->getItem("page[@type='projector']")}} itemscope="itemscope" itemtype="http://schema.org/Product" {{/if}}{{if $xml->getItem("page/@type") == (string) 'projector'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['73991::70925_projector_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'search'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['73991::70925_search_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'main'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['73991::70925_main_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'navigation'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['73991::70925_navigation_page_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'blog-list' or $xml->getItem("page/@type") == (string) 'blog-item'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['73991::70925_blog_c_class']}}{{/capture}}{{else}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['73991::70925_otherwise_c_class']}}{{/capture}}{{/if}}  class="{{$classAttributeTmp2}}">
						...
						{{if  $xml->getItem("page/calculations")}}
							<div class="affiliate_calculations_label">
								<span class="affiliate_calculations_label">{{$txt['59694::affiliate_calculations_label']}}</span>
							</div>
							{{if count( $xml->getList("page/calculations/payment"))}}
								<div class="affiliate_calculations">{{$txt['59694::affiliate_label_text']}}
									...
									<div class="affiliate_calculations_sub">
										<table {{assign "classAttributeTmp3" ""}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}affiliate_calculations{{/capture}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}ui-responsive table-stroke ui-table ui-table-reflow affiliate_calculations{{/capture}}  class="{{$classAttributeTmp3}}">
											<thead>
												<tr>
													<th id="affiliate_calculations_th1">{{$txt['59694::affiliate_calculations_th1']}}
													</th>
													<th id="affiliate_calculations_th2">{{$txt['59694::affiliate_calculations_th2']}}
													</th>
													<th id="affiliate_calculations_th3">{{$txt['59694::affiliate_calculations_th3']}}
													</th>
												</tr>
											</thead>
											<tbody>
												{{foreach from=$xml->getList("page/calculations/payment") item=loop307}}
													<tr {{assign "classAttributeTmp4" ""}}{{if $loop307@iteration % 2}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}affiliate_calculations_row1{{/capture}}{{else}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}affiliate_calculations_row2{{/capture}}{{/if}}  class="{{$classAttributeTmp4}}">
														<td class="affiliate_calculations_first">{{$loop307->getItemEscape("@date")}}
														</td>
														<td class="affiliate_calculations_secound">
															{{if 0  gt (string)  $loop307->getItem("@value")}}
																<span class="r">{{$loop307->getItemEscape("@value_formatted")}}</span>
															{{else}}
																<span class="g">
																	+{{$loop307->getItemEscape("@value_formatted")}}</span>
															{{/if}}
														</td>
														{{capture name="orderid" assign="orderid"}}
															{{if  $loop307->getItem("@orderid")}}
																{{$loop307->getItemEscape("@orderid")}}
															{{else}}
																{{$loop307->getItemEscape("@id")}}
															{{/if}}
														{{/capture}}
														<td class="affiliate_calculations_third">
															{{if  $loop307->getItem("@type")  == (string) 'gd'}}
																<span>{{$txt['59694::affiliate_gd']}}{{$orderid}}</span>
															{{elseif  $loop307->getItem("@type")  == (string) 'rd'}}
																<span class="r">{{$txt['59694::affiliate_rd']}}{{$orderid}}</span>
															{{elseif  $loop307->getItem("@type")  == (string) 'gi'}}
																<span>{{$txt['59694::affiliate_gi']}}{{$orderid}}</span>
															{{elseif  $loop307->getItem("@type")  == (string) 'ri'}}
																<span class="r">{{$txt['59694::affiliate_ri']}}{{$orderid}}</span>
															{{elseif  $loop307->getItem("@type")  == (string) 'gv'}}
																<span>{{$txt['59694::affiliate_gv']}}{{$orderid}}</span>
															{{elseif  $loop307->getItem("@type")  == (string) 'rv'}}
																<span class="r">{{$txt['59694::affiliate_rv']}}{{$orderid}}</span>
															{{elseif  $loop307->getItem("@type")  == (string) 'gc'}}
																<span>{{$txt['59694::affiliate_gc']}}</span>
															{{elseif  $loop307->getItem("@type")  == (string) 'rc'}}
																<span class="r">{{$txt['59694::affiliate_rc']}}</span>
															{{/if}}
														</td>
													</tr>
												{{/foreach}}
							...
							{{else}}
								<div class="affiliate_calculations_null">{{$txt['59694::affiliate_calculations_null']}}
								</div>
							{{/if}}
						{{/if}}
						...
						{{if count( $xml->getList("page/calculations/navigation/item"))}}
							{{capture name="lastItem" assign="lastItem"}}
								{{$xml->getItemEscape("page/calculations/navigation/item[position() = last()]/@counter")}}
							{{/capture}}
							<table class="affiliate_calculations_paging">
								<tr>
									<td class="affiliate_calculations_portions">
										{{if count( $xml->getList("page/calculations/navigation/portions"))  gt (string) 1}}
											<form class="affiliate_calculations_sort" action="settings.php" id="affiliate_calculations_sort">
												...
												<select class="affiliate_calculations_portions" name="affiliate_portions" onchange="submit();">
													{{foreach from=$xml->getList("page/calculations/navigation/portions") item=loop370}}
														<option value="{{$loop370->getItemEscape("@counter")}}" {{if $loop370->getItem("@selected") == (string) 'true'}} selected="selected" {{/if}}>{{$loop370->getItemEscape("@counter")}}
														</option>
													{{/foreach}}
												</select>
												<span class="affiliate_calculations_portions">{{$txt['59696::affiliate_calculations_002']}}</span>
											</form>
										{{/if}}
									</td>
													...
													<tr>
														<td class="aclc_paging_sub">
															{{if  $xml->getItem("page/calculations/navigation/prev")}}
																<a class="aclc_paging_first" href="{{$xml->getItem("page/calculations/navigation/path/@value")}}0">{{$txt['59696::affiliate_calculations_003']|escape:'iai'}}</a>
															{{/if}}
															{{if !( $xml->getItem("page/calculations/navigation/prev"))}}
																<span class="aclc_paging_first">{{$txt['59696::affiliate_calculations_004']|escape:'iai'}}</span>
															{{/if}}
															{{if  $xml->getItem("page/calculations/navigation/prev")}}
																<a class="aclc_paging_left" href="{{$xml->getItem("page/calculations/navigation/path/@value")}}{{$xml->getItemEscape("page/calculations/navigation/prev/@counter")}}">{{$txt['59696::affiliate_calculations_005']|escape:'iai'}}</a>
															{{/if}}
															{{if !( $xml->getItem("page/calculations/navigation/prev"))}}
																<span class="aclc_paging_left">{{$txt['59696::affiliate_calculations_006']|escape:'iai'}}</span>
															{{/if}}
															{{foreach from=$xml->getList("page/calculations/navigation/item") item=loop424}}
																{{if  $loop424->getItem("@gap")}}
																	<span class="aclc_paging_gap">{{$txt['59696::affiliate_calculations_007']}}</span>
																{{/if}}
																{{if !( $loop424->getItem("@type")  == (string) 'current') and !( $loop424->getItem("@gap"))}}
																	<a class="aclc_paging" href="{{$loop424->getItem("../path/@value")}}{{$loop424->getItemEscape("@counter") - 1}}" {{if $loop424@iteration == (string) $loop424@total }} id="last_of_page"{{/if}}>{{$loop424->getItemEscape("@counter")}}</a>
																{{/if}}
																{{if  $loop424->getItem("@type")  == (string) 'current'}}
																	<span class="aclc_paging_current">{{$loop424->getItemEscape("@counter")}}</span>
																{{/if}}
																{{if  $loop424@iteration   neq (string)  $loop424@total }}
																	{{$txt['59696::affiliate_calculations_008']}}
																{{/if}}
															{{/foreach}}
															{{if  $xml->getItem("page/calculations/navigation/prev") and  $xml->getItem("page/calculations/navigation/next")}}
																{{$txt['59696::affiliate_calculations_008']}}
															{{/if}}
															{{if  $xml->getItem("page/calculations/navigation/next")}}
																<a class="aclc_paging_right" href="{{$xml->getItem("page/calculations/navigation/path/@value")}}{{$xml->getItemEscape("page/calculations/navigation/next/@counter")}}">{{$txt['59696::affiliate_calculations_009']}}</a>
															{{/if}}
															{{if !( $xml->getItem("page/calculations/navigation/next"))}}
																<span class="aclc_paging_right">{{$txt['59696::affiliate_calculations_010']}}</span>
															{{/if}}
															{{if  $xml->getItem("page/calculations/navigation/next")}}
																<a class="aclc_paging_last" href="{{$xml->getItem("page/calculations/navigation/path/@value")}}{{$xml->getItemEscape("$lastItem - 1")}}">{{$txt['59696::affiliate_calculations_011']|escape:'iai'}}</a>
															{{/if}}
															{{if !( $xml->getItem("page/calculations/navigation/next"))}}
																<span class="aclc_paging_last">{{$txt['59696::affiliate_calculations_012']|escape:'iai'}}</span>
															{{/if}}
														</td>
													</tr>
												</table>
