								...
								<div class="affiliate_clients">
									<div class="affiliate_clients_sub">
										<table {{assign "classAttributeTmp4" ""}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}affiliate_clients{{/capture}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}ui-responsive table-stroke ui-table ui-table-reflow affiliate_clients{{/capture}}  class="{{$classAttributeTmp4}}">
											...
											<tbody>
												{{foreach from=$xml->getList("page/clients_list/login") item=loop304}}
													<tr {{assign "classAttributeTmp5" ""}}{{if $loop304@iteration % 2}}{{capture name="classAttributeTmp5" assign="classAttributeTmp5"}}affiliate_clients_row1{{/capture}}{{else}}{{capture name="classAttributeTmp5" assign="classAttributeTmp5"}}affiliate_clients_row2{{/capture}}{{/if}}  class="{{$classAttributeTmp5}}">
														<td class="affiliate_clients_first">
															<span class="affiliate_clients_name">
																<a data-mobile-class="btn-small" href="{{$loop304->getItemEscape("@link")}}">{{$loop304->getItemEscape("@nr")}}</a></span>
														</td>
						...
						{{if count( $xml->getList("page/clients_list/navigation/item"))}}
							<ul class="pagination pull-right">
								{{if count( $xml->getList("page/clients_list/navigation/item"))  gt (string) 1}}
									...
									{{/if}}
									{{foreach from=$xml->getList("page/clients_list/navigation/item") item=loop358}}
										{{if  $loop358->getItem("@gap")}}
											<li class="disabled">
												<span>{{$txt['91987::affiliate_clients_list_007']}}</span>
											</li>
										{{/if}}
										{{if !( $loop358->getItem("@type")  == (string) 'current') and !( $loop358->getItem("@gap"))}}
											<li>
												<a href="{{$loop358->getItem("../path/@value")}}{{$loop358->getItemEscape("@counter") - 1}}">{{$loop358->getItemEscape("@counter")}}</a>
											</li>
										{{/if}}
										{{if  $loop358->getItem("@type")  == (string) 'current'}}
											<li class="active">
												<span>{{$loop358->getItemEscape("@counter")}}</span>
											</li>
										{{/if}}
									{{/foreach}}
