						...
						{{if count( $xml->getList("/shop/page/blogitems/blogitem"))  gt (string) 0}}
							<div id="blog_list" class="mb-4 article_wrapper">
								<div class="row article_sub_wrapper">
									{{foreach from=$xml->getList("/shop/page/blogitems/blogitem") item=loop331}}
										<div class="article_element_wrapper col-md-6 col-12">
											<div class="date datasquare">{{$loop331->getItem("date")}}
											</div>
											{{if  $loop331->getItem("more/@href") and !( $loop331->getItem("more/@href") == (string)'')}}
												<h3 class="article_name_wrapper">
													<a class="article_name" href="{{$loop331->getItemEscape("more/@href")}}" title="{{$loop331->getItem("title")}}">{{$loop331->getItem("title")}}</a>
												</h3>
											{{else}}
												{{if  $loop331->getItem("title")  neq (string) ''}}
													<h3 class="article_name_wrapper">
														<span class="article_name">{{$loop331->getItem("title")}}</span>
													</h3>
												{{/if}}
											{{/if}}
											{{if  $loop331->getItem("image/@src") and  $loop331->getItem("image/@src") neq (string)''}}
												{{if  $loop331->getItem("@link")}}
													{{literal}}
													<a class="article_image_wrapper" href="{{/literal}}{{$loop331->getItemEscape("@link")}}{{literal}}" {{/literal}}{{else}}{{literal}}
<span class="article_image_wrapper" {{/literal}}{{/if}}{{literal}}>{{/literal}}
														<img src="/gfx/__IAI_TEMPLATE_LANG_639_1__/loader.gif?r=1570606968" data-src-small="{{$loop331->getItem("image/@src")}}" data-src="{{$loop331->getItem("image/@src")}}" class="article_image b-lazy" alt="{{$loop331->getItem("title/text()")}}"></img>
														{{if  $loop331->getItem("@link")}}
															{{literal}}</a>{{/literal}}
													{{else}}
											...
											<div class="article_text_wrapper">
												<div class="description_text">{{$loop331->getItem("description")}}
												</div>
												{{if  $loop331->getItem("more/@href") and !( $loop331->getItem("more/@href") == (string)'') and $main_blog_list_see_more }}
													<div class="see_more_wrapper">
														<a class="see_more_link" href="{{$loop331->getItemEscape("more/@href")}}" title="{{$loop331->getItem("title")}}">{{$txt['107751::53698_xx']}}</a>
													</div>
												{{/if}}
											</div>
										</div>
						...
						{{if count( $xml->getList("page/blogitems/navigation/item"))}}
							<ul class="pagination pull-right">
								{{if count( $xml->getList("page/blogitems/navigation/item"))  gt (string) 1}}
									...
									{{/if}}
									{{foreach from=$xml->getList("page/blogitems/navigation/item") item=loop421}}
										{{if  $loop421->getItem("@gap")}}
											<li class="disabled">
												<span>{{$txt['107757::affiliate_calculations_007']}}</span>
											</li>
										{{/if}}
										{{if !( $loop421->getItem("@type")  == (string) 'current') and !( $loop421->getItem("@gap"))}}
											<li>
												<a href="{{$loop421->getItem("../path/@value")}}{{$loop421->getItemEscape("@counter") - 1}}">{{$loop421->getItemEscape("@counter")}}</a>
											</li>
										{{/if}}
										{{if  $loop421->getItem("@type")  == (string) 'current'}}
											<li class="active">
												<span>{{$loop421->getItemEscape("@counter")}}</span>
											</li>
										{{/if}}
									{{/foreach}}
