						...
						{{if  $xml->getItem("page/@type")  == (string) 'return_products' and  $xml->getItem("page/@display")  == (string) 'stock'}}
							<div class="step4a">
								...
								<form class="pickup_return_list_form" method="get">
									{{if count( $xml->getList("page/pickups/pickup"))  gt (string) 1}}
										{{foreach from=$xml->getList("page/pickups/pickup") item=loop359}}
											<div {{assign "classAttributeTmp4" ""}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}pickupl_return{{/capture}}{{if $loop359->getItem("@checked") == (string) 'true'}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}pickupl_return active{{/capture}}{{/if}} id="ps_{{$loop359->getItemEscape("@value")}}"  class="{{$classAttributeTmp4}}">
												<input style="display:none;" type="radio" name="stock" {{if $loop359->getItem("@checked") == (string) 'true'}} checked="checked"{{/if}} value="{{$loop359->getItemEscape("id")}}"></input>
												<div class="pickupl_return_sub">
													<div class="pickupl_return_header">
														<span class="pickupl_return_name">{{$loop359->getItemEscape("name")}}</span>
													</div>
													<div class="pickupl_return_address">
														<span class="pickup_return_address" style="display:none">{{$loop359->getItemEscape("address")}}</span>{{$loop359->getItemEscape("address")}}
														<span class="pickupl_return_city">{{$loop359->getItemEscape("zipcode")}}{{literal}}{{/literal}}{{$loop359->getItemEscape("city")}}</span>
													</div>
													<div class="pickupl_return_link">
														<a target="_blank" href="{{$loop359->getItemEscape("link")}}" title="{{$txt['107784::pickupl_return_linktitle']}}">{{$txt['107784::pickupl_return_link']}}</a>
													</div>
													{{if  $loop359->getItem("info")}}
														<div class="pickupl_return_info">{{$loop359->getItemEscape("info")}}
														</div>
													{{/if}}
												</div>
											</div>
										{{/foreach}}
									{{else}}
										{{foreach from=$xml->getList("page/pickups/pickup") item=loop399}}
											<div class="pickupl_return_one" id="ps_{{$loop399->getItemEscape("@value")}}">
												<input style="display:none;" type="radio" name="stock" checked="checked" value="{{$loop399->getItemEscape("id")}}"></input>
												<div class="pickupl_return_one_sub">
													<div class="pickupl_return_one_link">
														<a target="_blank" data-tablet-class="btn-small" data-mobile-class="btn-small" href="{{$loop399->getItemEscape("link")}}" title="{{$txt['107784::pickupl_return_one_linktitle']}}">{{$txt['107784::pickupl_return_one_link']}}</a>
													</div>
													<div class="pickupl_return_one_header">
														<span class="pickupl_return_one_name">{{$loop399->getItemEscape("name")}}</span>
													</div>
													<div class="pickupl_return_one_address">{{$loop399->getItemEscape("address")}}{{literal}}{{/literal}}
														<span class="pickupl_return_one_city">{{$loop399->getItemEscape("zipcode")}}{{literal}}{{/literal}}{{$loop399->getItemEscape("city")}}</span>
													</div>
													{{if  $loop399->getItem("info")}}
														<div class="pickupl_return_one_info">{{$loop399->getItemEscape("info")}}
														</div>
													{{/if}}
												</div>
											</div>
