									...
									<form class="returns_form" method="post" action="{{$xml->getItemEscape("page/formactionlink/@url")}}">
										...
										<div class="order2_products">
											...
											<table class="order2_table">
												...
												{{foreach from=$xml->getList("page/returnproducts/products/product") item=loop470}}
													<tr>
														<td class="order2_name">
															{{if  $loop470->getItem("@gratis") == (string)'y'}}
																<span class="order2_name">{{$loop470->getItem("@name")}}</span>
															{{else}}
																<a class="order2_name" href="{{$loop470->getItemEscape("@link")}}">{{$loop470->getItem("@name")}}</a>
															{{/if}}
															{{if !( $loop470->getItem("size/@type") == (string)'uniw')}}
																<div class="order2_size_desc">{{$txt['107797::51000_013']}}{{$loop470->getItemEscape("size/@description")}}
																</div>
															{{/if}}
															{{if ( $loop470->getItem("version")) and (!( $loop470->getItem("version/@product_name") == (string)''))}}
																<div class="order2_size_desc">{{$loop470->getItemEscape("version/@name")}}{{$txt['107797::51000_0kro']}}{{$loop470->getItemEscape("version/@product_name")}}
																</div>
															{{/if}}
															{{if  $loop470->getItem("@required_from_special_offer")  == (string) 'true' and $required_from_special_offer  neq (string) ''}}
																{{if  $loop470->getItem("@required_from_special_offer")  == (string) 'true'}}
														...
														<td class="order2_price">
															{{if  $loop470->getItem("@forpoints")}}
																{{$loop470->getItemEscape("@points_for_one")}}{{$txt['107797::51000_993']}}
															{{/if}}
															{{if !( $loop470->getItem("@forpoints"))}}
																{{$loop470->getItemEscape("@detalprice_formatted")}}
															{{/if}}
														</td>
														<td class="order2_quantity">{{$loop470->getItemEscape("@quantity")}}{{literal}}{{/literal}}{{$loop470->getItemEscape("@worth_unit")}}
														</td>
														<td class="order2_bruttoworth">
															{{if  $loop470->getItem("@gratis") == (string)'y'}}
																{{$txt['107797::51000_gratis']}}
															{{elseif !( $loop470->getItem("@forpoints"))}}
																{{$loop470->getItemEscape("@bruttoworth_formatted")}}
															{{elseif  $loop470->getItem("@forpoints")}}
																{{$loop470->getItemEscape("@points")}}{{$txt['107797::51000_993']}}
															{{/if}}
														</td>
													</tr>
												{{/foreach}}
