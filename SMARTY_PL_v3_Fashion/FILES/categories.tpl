						...
						<div class="categories-list_wrapper">
							{{if  $xml->getItem("page/categories-list/@mode") == (string)'2'}}
								<ul class="categories-list_level1">
									{{foreach from=$xml->getList("/shop/navigation2/item") item=loop250}}
										<li class="categories-list_level1">
											{{if  $loop250->getItem("@link")}}
												<a class="categories-list_level1" href="{{$loop250->getItemEscape("@link")}}" title="{{$loop250->getItemEscape("@name")}}">{{$loop250->getItem("@name")}}</a>
											{{else}}
												<span class="categories-list_level1">{{$loop250->getItem("@name")}}</span>
											{{/if}}
											{{if  $loop250->getItem("item")}}
												...
												<ul class="categories-list_level2">
													{{foreach from=$loop250->getList("item") item=loop270}}
														<li class="categories-list_level2">
															{{if  $loop270->getItem("@link")}}
																<a class="categories-list_level2" href="{{$loop270->getItemEscape("@link")}}" title="{{$loop270->getItemEscape("@name")}}">{{$loop270->getItem("@name")}}</a>
															{{else}}
																<span class="categories-list_level2">{{$loop270->getItem("@name")}}</span>
															{{/if}}
															{{if  $loop270->getItem("item")}}
																<span {{assign "classAttributeTmp3" ""}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}more_categories{{/capture}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}more_categories d-sm-none{{/capture}}  class="{{$classAttributeTmp3}}">{{$txt['107766::40002_000']}}</span>
																<ul class="categories-list_level3">
																	{{foreach from=$loop270->getList("item") item=loop290}}
																		<li class="categories-list_level3">
																			{{if  $loop290->getItem("@link")}}
																				<a class="categories-list_level3" href="{{$loop290->getItemEscape("@link")}}" title="{{$loop290->getItemEscape("@name")}}">{{$loop290->getItem("@name")}}</a>
																			{{else}}
																				<span class="categories-list_level3">{{$loop290->getItem("@name")}}</span>
																			{{/if}}
																		</li>
																	{{/foreach}}
																</ul>
							...
							{{else}}
								<ul class="categories-list_level1">
									{{foreach from=$xml->getList("/shop/navigation/item") item=loop320}}
										<li class="categories-list_level1">
											{{if  $loop320->getItem("@link")}}
												<a class="categories-list_level1" href="{{$loop320->getItemEscape("@link")}}" title="{{$loop320->getItemEscape("@name")}}">{{$loop320->getItem("@name")}}</a>
											{{else}}
												<span class="categories-list_level1">{{$loop320->getItem("@name")}}</span>
											{{/if}}
											{{if  $loop320->getItem("item")}}
												<span {{assign "classAttributeTmp3" ""}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}more_categories{{/capture}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}more_categories d-sm-none{{/capture}}  class="{{$classAttributeTmp3}}">{{$txt['107766::40002_000']}}</span>
												<ul class="categories-list_level2">
													{{foreach from=$loop320->getList("item") item=loop340}}
														<li class="categories-list_level2">
															{{if  $loop340->getItem("@link")}}
																<a class="categories-list_level2" href="{{$loop340->getItemEscape("@link")}}" title="{{$loop340->getItemEscape("@name")}}">{{$loop340->getItem("@name")}}</a>
															{{else}}
																<span class="categories-list_level2">{{$loop340->getItem("@name")}}</span>
															{{/if}}
															{{if  $loop340->getItem("item")}}
																<span {{assign "classAttributeTmp3" ""}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}more_categories{{/capture}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}more_categories d-sm-none{{/capture}}  class="{{$classAttributeTmp3}}">{{$txt['107766::40002_000']}}</span>
																<ul class="categories-list_level3">
																	{{foreach from=$loop340->getList("item") item=loop360}}
																		<li class="categories-list_level3">
																			{{if  $loop360->getItem("@link")}}
																				<a class="categories-list_level3" href="{{$loop360->getItemEscape("@link")}}" title="{{$loop360->getItemEscape("@name")}}">{{$loop360->getItem("@name")}}</a>
																			{{else}}
																				<span class="categories-list_level3">{{$loop360->getItem("@name")}}</span>
																			{{/if}}
																		</li>
																	{{/foreach}}
																</ul>
