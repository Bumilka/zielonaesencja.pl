							...
							<div class="affiliate_client">
								<div class="affiliate_client_sub">
									<table {{assign "classAttributeTmp4" ""}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}affiliate_client{{/capture}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}ui-responsive table-stroke ui-table ui-table-reflow affiliate_client{{/capture}}  class="{{$classAttributeTmp4}}">
										...
										<tbody>
											{{foreach from=$xml->getList("page/client/item") item=loop308}}
												<tr {{assign "classAttributeTmp5" ""}}{{if $loop308@iteration % 2}}{{capture name="classAttributeTmp5" assign="classAttributeTmp5"}}affiliate_client_row1{{/capture}}{{else}}{{capture name="classAttributeTmp5" assign="classAttributeTmp5"}}affiliate_client_row2{{/capture}}{{/if}}  class="{{$classAttributeTmp5}}">
													<td class="affiliate_client_first">
														{{if  $loop308->getItem("@orderid")}}
															{{$loop308->getItemEscape("@orderid")}}
														{{else}}
															{{$loop308->getItemEscape("@id")}}
														{{/if}}
													</td>
													<td>{{$loop308->getItemEscape("@date")}}
													</td>
													<td>{{$loop308->getItemEscape("@dispatch")}}
													</td>
													<td>
														<span class="affiliate_client_price">{{$loop308->getItemEscape("@cost_formatted")}}</span>
													</td>
													<td>{{$loop308->getItemEscape("@numprod")}}
													</td>
													<td>
														<span class="affiliate_client_price">{{$loop308->getItemEscape("@prepaid_formatted")}}</span>
													</td>
													<td>{{$loop308->getItemEscape("@status")}}
													</td>
													<td>{{$loop308->getItemEscape("@affiliate")}}
													</td>
													<td>{{$loop308->getItemEscape("@commission_formatted")}}
													</td>
												</tr>
											{{/foreach}}
										</tbody>
						...
						{{if count( $xml->getList("page/client/navigation/item"))}}
							<ul class="pagination pull-right">
								{{if count( $xml->getList("page/client/navigation/item"))  gt (string) 1}}
									...
									{{/if}}
									{{foreach from=$xml->getList("page/client/navigation/item") item=loop383}}
										{{if  $loop383->getItem("@gap")}}
											<li class="disabled">
												<span>{{$txt['91985::affiliate_client_007']}}</span>
											</li>
										{{/if}}
										{{if !( $loop383->getItem("@type")  == (string) 'current') and !( $loop383->getItem("@gap"))}}
											<li>
												<a href="{{$loop383->getItem("../path/@value")}}{{$loop383->getItemEscape("@counter") - 1}}">{{$loop383->getItemEscape("@counter")}}</a>
											</li>
										{{/if}}
										{{if  $loop383->getItem("@type")  == (string) 'current'}}
											<li class="active">
												<span>{{$loop383->getItemEscape("@counter")}}</span>
											</li>
										{{/if}}
									{{/foreach}}
