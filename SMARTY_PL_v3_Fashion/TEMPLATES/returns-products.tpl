									...
									<form class="returns_productslist" action="/returns-method.php?type=multiproduct" method="post" enctype="multipart/form-data">
										<div id="basketedit_productslist" class="basketedit_productslist return_productslist">
											<table>
												...
												{{foreach from=$xml->getList("page/products/product") item=loop383}}
													<tr class="productslist_item">
														<td class="productslist_product_photo">
															<a href="{{$loop383->getItemEscape("@link")}}">
																<img {{if $setIconLarge}} src="{{$loop383->getItemEscape("icon")}}" {{else}} src="{{$loop383->getItemEscape("iconsmall")}}" {{/if}} alt="{{$loop383->getItem("name/text()")}}"></img></a>
														</td>
														<td class="productslist_product_name">
															<h3>
																<a href="{{$loop383->getItemEscape("@link")}}">{{$loop383->getItem("name/text()")}}</a>
															</h3>
															{{if !( $loop383->getItem("@barcode")  == (string)'')}}
																<div class="productslist_product_barcode">
																	<span>{{$txt['107781::50001_99']}}</span>{{$loop383->getItemEscape("@barcode")}}
																</div>
															{{/if}}
															{{if !( $loop383->getItem("size/@type") == (string)'uniw')}}
																<div class="productslist_product_size">
																	<span>{{$txt['107781::50001_004']}}</span>{{$loop383->getItemEscape("size/@description")}}
																</div>
															{{/if}}
															{{if ( $loop383->getItem("version")) and (!( $loop383->getItem("version/@product_name") == (string)''))}}
																<div class="productslist_product_version">
																	<span>{{$loop383->getItemEscape("version/@name")}}:</span>{{$loop383->getItemEscape("version/@product_name")}}
																</div>
															{{/if}}
															{{if $allowReturnInfo and  $loop383->getItem("@allowReturn") == (string)'0'}}
															...
															{{/if}}
															{{if  $loop383->getItem("/shop/page/products/product/@required_from_special_offer")  == (string) 'true' and $required_from_special_offer  neq (string) ''}}
																{{if  $loop383->getItem("@required_from_special_offer")  == (string) 'true'}}
														...
														<td class="productslist_product_prices">
															{{if  $loop383->getItem("size/@ordered_points")}}
																<del>
																	{{if ( $loop383->getItem("/shop/basket/@wholesaler")  == (string) 'true') and !($wholesalerPrice)}}
																		{{$loop383->getItemEscape("price/@price_net_formatted")}}
																	{{else}}
																		{{$loop383->getItemEscape("price/@price_formatted")}}
																	{{/if}}
																	</del>
																<span>{{$loop383->getItemEscape("price/@points")}}{{$txt['107781::55925_pkt']}}</span>
															{{else}}
																{{if  $loop383->getItem("price/@beforerebate")}}
																	<del>{{$loop383->getItemEscape("price/@beforerebate_formatted")}}</del>
																{{/if}}
																<span>
																	{{if ( $loop383->getItem("/shop/basket/@wholesaler")  == (string) 'true') and !($wholesalerPrice)}}
																		{{$loop383->getItemEscape("price/@price_net_formatted")}}
														...
														<td class="productslist_product_quantity">
															{{if $allowReturnInfoPoint and  $loop383->getItem("@allowReturn") == (string)'0' and  $loop383->getItem("@for_points") == (string)'true'}}
																colspan="2"
																<div class="productslist_product_info">{{$allowReturnInfoPoint}}
																</div>
															{{elseif $allowReturnInfoGratis and  $loop383->getItem("@allowReturn") == (string)'0' and  $loop383->getItem("@gratis") == (string)'t'}}
																colspan="2"
																<div class="productslist_product_info">{{$allowReturnInfoGratis}}
																</div>
															{{elseif $allowReturnInfo and  $loop383->getItem("@allowReturn") == (string)'0'}}
																colspan="2"
																<div class="productslist_product_info">{{$allowReturnInfo}}
																</div>
															{{else}}
																<div>
																	{{if  $loop383->getItem("change_group/@id")}}
																		class="productslist_product_quantityproductslist_product_quantity_change"
																		<input type="hidden" name="product[{{$loop383@iteration}}]" value="{{$loop383->getItemEscape("@id")}}"></input>
																		<input type="hidden" name="size[{{$loop383@iteration}}]" value="{{$loop383->getItemEscape("size/@type")}}"></input>
																		{{if  $loop383->getItem("additional/@hash")}}
																			<input type="hidden" name="additional[{{$loop383@iteration}}]" value="{{$loop383->getItemEscape("additional/@hash")}}"></input>
																		{{/if}}
																		{{if  $loop383->getItem("size/@ordered_points")}}
																			<input type="hidden" value="1" name="forpoints[{{$loop383@iteration}}]"></input>
																		{{/if}}
																		<input {{assign "valueAttributeTmp6" ""}}  type="hidden" name="set_quantity[{{$loop383@iteration}}]" {{if $loop383->getItem("size/@ordered_points")}}{{capture name="valueAttributeTmp6" assign="valueAttributeTmp6"}}{{$loop383->getItemEscape("size/@ordered_points")}}{{/capture}}{{else}}{{capture name="valueAttributeTmp6" assign="valueAttributeTmp6"}}{{$loop383->getItemEscape("size/@ordered")}}{{/capture}}{{/if}}  value="{{$valueAttributeTmp6}}"></input>
																		<strong>
																			{{if  $loop383->getItem("size/@ordered_points")}}
																				{{$loop383->getItemEscape("size/@ordered_points")}}
																			{{else}}
																				{{$loop383->getItemEscape("size/@ordered")}}
																			{{/if}}
																			<span>
																				{{if !( $loop383->getItem("size/@ordered")  gt (string) 1)}}
																					{{$loop383->getItemEscape("size/@unit_singular")}}
																				{{else}}
																					{{$loop383->getItemEscape("size/@unit")}}
																				{{/if}}
																				</span></strong>
																		{{if  $loop383->getItem("change_group/@changelink")}}
																			<a class="productslist_change_product" href="{{$loop383->getItemEscape("change_group/@changelink")}}">{{$txt['107781::50001_99wy']}}</a>
																		{{/if}}
																	{{else}}
																		<input type="hidden" name="size[{{$loop383@iteration}}]" value="{{$loop383->getItemEscape("size/@type")}}"></input>
																		{{if  $loop383->getItem("additional/@hash")}}
																		...
																		<span class="productslist_quantity">
																			...
																			<input type="text" name="set_quantity[{{$loop383@iteration}}]" value="{{$loop383->getItemEscape("@selected_quantity")}}"></input>
																			<input type="hidden" name="data_amount" value="{{$loop383->getItemEscape("@quantity")}}"></input>
																			<input type="hidden" name="data_unit_sellby" value="{{$loop383->getItemEscape("size/@unit_sellby")}}"></input>
																			<input type="hidden" name="data_unit" value="{{$loop383->getItemEscape("size/@unit")}}"></input>
																			<input type="hidden" name="data_unit_precision" value="{{$loop383->getItemEscape("size/@unit_precision")}}"></input>
																		...
																		{{if !( $loop383->getItem("size/@unit_sellby")  == (string) '1')}}
																			<div class="productslist_product_sellby_info">{{$txt['107781::50001_900bb_xxx']}}{{$loop383->getItemEscape("size/@unit_sellby")}}{{literal}}{{/literal}}
																				{{if !( $loop383->getItem("size/@ordered")  gt (string) 1)}}
																					{{$loop383->getItemEscape("size/@unit_singular")}}
																				{{else}}
																					{{$loop383->getItemEscape("size/@unit")}}
														...
														<td class="productslist_product_action" {{if $allowReturnInfo and $loop383->getItem("@allowReturn") == (string)'0'}} style="display:none;" {{/if}}>
															<input type="checkbox" name="product[{{$loop383@iteration}}]" value="{{$loop383->getItemEscape("@id")}}" {{if $loop383->getItem("@checked") == (string) 'true' or $loop383->getItem("@selected_quantity") gt (string) 0}} checked="checked"{{/if}}></input>
														</td>
													</tr>
