								...
								<div class="affiliate_calculations">
									<div class="affiliate_calculations_sub">
										<table {{assign "classAttributeTmp3" ""}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}affiliate_calculations{{/capture}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}ui-responsive table-stroke ui-table ui-table-reflow affiliate_calculations{{/capture}}  class="{{$classAttributeTmp3}}">
											...
											<tbody>
												{{foreach from=$xml->getList("page/calculations/payment") item=loop323}}
													<tr {{assign "classAttributeTmp4" ""}}{{if $loop323@iteration % 2}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}affiliate_calculations_row1{{/capture}}{{else}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}affiliate_calculations_row2{{/capture}}{{/if}}  class="{{$classAttributeTmp4}}">
														<td class="affiliate_calculations_first">{{$loop323->getItemEscape("@date")}}
														</td>
														<td>{{$loop323->getItemEscape("@available_from")}}
														</td>
														<td>{{$loop323->getItemEscape("@valid_until")}}
														</td>
														<td class="affiliate_calculations_secound">
															{{if 0  gt (string)  $loop323->getItem("@value")}}
																<span class="r">{{$loop323->getItemEscape("@value_formatted")}}</span>
															{{else}}
																<span class="g">
																	+{{$loop323->getItemEscape("@value_formatted")}}</span>
															{{/if}}
														</td>
														{{capture name="orderid" assign="orderid"}}
															{{if  $loop323->getItem("@orderid")}}
																{{$loop323->getItemEscape("@orderid")}}
															{{else}}
																{{$loop323->getItemEscape("@id")}}
															{{/if}}
														{{/capture}}
														<td class="affiliate_calculations_third">
															{{if  $loop323->getItem("@type")  == (string) 'gd'}}
																<span>{{$txt['107730::affiliate_gd']}}{{$orderid}}</span>
															{{elseif  $loop323->getItem("@type")  == (string) 'rd'}}
																<span class="r">{{$txt['107730::affiliate_rd']}}{{$orderid}}</span>
															{{elseif  $loop323->getItem("@type")  == (string) 'gi'}}
																<span>{{$txt['107730::affiliate_gi']}}{{$orderid}}</span>
															{{elseif  $loop323->getItem("@type")  == (string) 'ri'}}
																<span class="r">{{$txt['107730::affiliate_ri']}}{{$orderid}}</span>
															{{elseif  $loop323->getItem("@type")  == (string) 'gv'}}
																<span>{{$txt['107730::affiliate_gv']}}{{$orderid}}</span>
															{{elseif  $loop323->getItem("@type")  == (string) 'rv'}}
																<span class="r">{{$txt['107730::affiliate_rv']}}{{$orderid}}</span>
															{{elseif  $loop323->getItem("@type")  == (string) 'gc'}}
																<span>{{$txt['107730::affiliate_gc']}}</span>
															{{elseif  $loop323->getItem("@type")  == (string) 'rc'}}
																<span class="r">{{$txt['107730::affiliate_rc']}}</span>
															{{elseif  $loop323->getItem("@type")  == (string) 'rr'}}
						...
						{{if count( $xml->getList("page/calculations/navigation/item"))}}
							<ul class="pagination pull-right">
								{{if count( $xml->getList("page/calculations/navigation/item"))  gt (string) 1}}
									...
									{{/if}}
									{{foreach from=$xml->getList("page/calculations/navigation/item") item=loop410}}
										{{if  $loop410->getItem("@gap")}}
											<li class="disabled">
												<span>{{$txt['91956::affiliate_calculations_007']}}</span>
											</li>
										{{/if}}
										{{if !( $loop410->getItem("@type")  == (string) 'current') and !( $loop410->getItem("@gap"))}}
											<li>
												<a href="{{$loop410->getItem("../path/@value")}}{{$loop410->getItemEscape("@counter") - 1}}">{{$loop410->getItemEscape("@counter")}}</a>
											</li>
										{{/if}}
										{{if  $loop410->getItem("@type")  == (string) 'current'}}
											<li class="active">
												<span>{{$loop410->getItemEscape("@counter")}}</span>
											</li>
										{{/if}}
									{{/foreach}}
