						...
						{{if  $xml->getItem("page/@type")  == (string) 'return_products' and  $xml->getItem("page/@display")  == (string) 'courier'}}
							<div class="step3d">
								...
								<form class="return_delivery_form row" action="" method="post" action="{{$xml->getItemEscape("page/formactionlink/@url")}}">
									{{foreach from=$xml->getList("page/shipping/delivery") item=loop350}}
										<div class="col-12 col-sm-6 col-lg-4">
											<div {{assign "classAttributeTmp4" ""}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}return_delitem{{/capture}} id="delivery_{{$loop350->getItemEscape("@id")}}" {{if $loop350->getItem("@checked") == (string) 'true'}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}return_delitem active{{/capture}}{{/if}}  class="{{$classAttributeTmp4}}">
												<div class="order1_delitem_sub">
													<span class="order1_delitem_radio" style="display:none;">
														<input type="radio" name="shipping" class="order1_delitem_radio" value="{{$loop350->getItemEscape("@id")}}" {{if $loop350->getItem("@checked") == (string) 'true'}} checked="checked"{{/if}} title="{{$loop350->getItemEscape("@comment")}}"></input></span>
													<div class="order1_delitem_header">
														{{if  $loop350->getItem("@icon")}}
															<img class="order1_delitem_icon" src="{{$loop350->getItemEscape("@icon")}}" alt="{{$loop350->getItemEscape("@name")}}" title="{{$loop350->getItemEscape("@name")}}"></img>
														{{/if}}
														<span class="order1_delitem_name">{{$loop350->getItemEscape("@name")}}
															<div class="order1_delitem_time">
																<span class="order1_delitem_time">{{$txt['107769::53687_0990']}}
																	{{if ( $loop350->getItem("@deliverytime_days")  == (string) 0) and ( $loop350->getItem("@deliverytime_hours")  == (string) 0) and ( $loop350->getItem("@deliverytime_minutes")  == (string) 0)}}
																		{{$txt['107769::53687_0991a']}}
																	{{else}}
																		{{if  $loop350->getItem("@deliverytime_days")  gt (string) 0}}
																			{{$loop350->getItemEscape("@deliverytime_days")}}
																			{{if  $loop350->getItem("@deliverytime_days")  == (string)'1'}}
																				{{$txt['107769::53706_003c']}}
																			{{else}}
																				{{$txt['107769::53706_003b']}}
																			{{/if}}
																		{{/if}}
																		{{if  $loop350->getItem("@deliverytime_hours")  gt (string) 0}}
																			{{$loop350->getItemEscape("@deliverytime_hours")}}{{$txt['107769::53706_003godz']}}
																		{{/if}}
																		{{if  $loop350->getItem("@deliverytime_minutes")  gt (string) 0}}
																			{{$loop350->getItemEscape("@deliverytime_minutes")}}{{$txt['107769::53706_003min']}}
																		{{/if}}
																	{{/if}}
																	</span>
															</div>
															<div class="order1_delitem_comment" style="display:none;" id="delivery_{{$loop350->getItemEscape("@id")}}">
															</div></span>
													</div>
													{{if !( $loop350->getItem("/shop/page/order1/shipping/@hide_cost") == (string)'true')}}
														<div class="order1_delitem_pricebox">
															{{if  $loop350->getItem("@points_enabled")  == (string) 'false'}}
																<label class="order1_delitem_price">
																	<input type="checkbox" name="" value="" checked="checked" {{if $loop350->getItem("@minworthreached") == (string) 'false'}} disabled="disabled"{{/if}}></input>{{$loop350->getItemEscape("@cost_formatted")}}
																</label>
																<label class="order1_delitem_price_point_disabled showTip_order1" title="{{$txt['107769::53687_011nopoints']}}">
																	<input class="showTip_order1" type="checkbox" disabled="disabled" title="{{$txt['107769::53687_011nopoints']}}"></input>{{$loop350->getItemEscape("@points_cost")}}{{$txt['107769::order1_form_pointsok3']}}
																</label>
															{{elseif  $loop350->getItem("@points_enabled")  == (string) 'true'}}
																<label class="order1_delitem_price">
																	<input type="checkbox" name="" value="" checked="checked" {{if $loop350->getItem("@minworthreached") == (string) 'false'}} disabled="disabled"{{/if}}></input>{{$loop350->getItemEscape("@cost_formatted")}}
																</label>
																<label class="order1_delitem_price_point">
																	<input type="checkbox" name="" value="" {{if $loop350->getItem("@minworthreached") == (string) 'false'}} disabled="disabled"{{/if}}></input>{{$loop350->getItemEscape("@points_cost")}}{{$txt['107769::order1_form_pointsok3']}}
																</label>
															{{else}}
																<span class="order1_delitem_pricetitle">{{$txt['107769::50000_001']}}</span>
																<span class="order1_delitem_price" {{if $loop350->getItem("@cost") gt (string) 0}}{{if $loop350->getItem("@limitfree") gt (string) 0}} title="{{$txt['107769::53687_011']}}{{$loop350->getItemEscape("@limitfree_formatted")}}{{$txt['107769::53687_012']}}"{{/if}}{{$loop350->getItemEscape("@cost_formatted")}}{{else}} {{$txt['107769::53687_014']}} {{/if}}></span>
															{{/if}}
														</div>
													{{/if}}
												</div>
