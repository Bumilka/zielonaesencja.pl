						...
						<div class="client_order">
							...
							{{if  $xml->getItem("/shop/page/returns/item")}}
								<table {{assign "classAttributeTmp3" ""}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}orders{{/capture}} cellspacing="0" cellpadding="0"{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}ui-responsive table-stroke ui-table ui-table-reflow orders{{/capture}}  class="{{$classAttributeTmp3}}">
									...
									<tbody>
										{{foreach from=$xml->getList("/shop/page/returns/item") item=loop320}}
											<tr>
												<td class="ord">
													{{if $setSerialNumber}}
														{{$loop320->getItemEscape("@number")}}
													{{else}}
														{{$loop320->getItemEscape("@id")}}
													{{/if}}
												</td>
												<td>{{$loop320->getItemEscape("@date")}}
												</td>
												<td class="ord">{{$loop320->getItemEscape("@cost_formatted")}}
												</td>
												<td>
													{{if  $loop320->getItem("@status_name")  neq (string) ''}}
														{{$loop320->getItem("@status_name")}}
													{{elseif  $loop320->getItem("@status") == (string)1}}
														{{$txt['69371::status_1']}}
													{{elseif  $loop320->getItem("@status") == (string)2}}
														{{$txt['69371::status_2']}}
													{{elseif  $loop320->getItem("@status") == (string)3}}
														{{$txt['69371::status_3']}}
													{{elseif  $loop320->getItem("@status") == (string)13}}
														{{$txt['69371::status_13']}}
													{{elseif  $loop320->getItem("@status") == (string)14}}
														{{$txt['69371::status_14']}}
													{{elseif  $loop320->getItem("@status") == (string)15}}
														{{$txt['69371::status_15']}}
													{{elseif  $loop320->getItem("@status") == (string)16}}
														{{$txt['69371::status_16']}}
													{{elseif  $loop320->getItem("@status") == (string)17}}
														{{$txt['69371::status_17']}}
													{{elseif  $loop320->getItem("@status") == (string)18}}
														{{$txt['69371::status_18']}}
													{{/if}}
												</td>
												<td class="ord">
													<a class="orders_more" data-tablet-class="btn-small" data-mobile-class="btn-small" href="{{$loop320->getItemEscape("@url")}}">{{$txt['69371::51900_018']}}</a>
												</td>
											</tr>
										{{/foreach}}
									</tbody>
