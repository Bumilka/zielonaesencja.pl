								...
								<form class="returm_return_list_form" method="post" novalidate="novalidate">
									<div class="table_display">
										{{if count( $xml->getList("page/repayment_methods/repayment_method"))  gt (string) 1}}
											{{foreach from=$xml->getList("page/repayment_methods/repayment_method") item=loop364}}
												<div {{assign "classAttributeTmp3" ""}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}return_form{{/capture}}{{if $loop364->getItem("@additional_field_name")}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}return_form disable_button{{/capture}}{{if $loop364->getItem("@checked") == (string)'true'}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}return_form disable_button active{{/capture}}{{/if}}{{/if}}{{if $loop364->getItem("@checked") == (string)'true'}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}return_form active{{/capture}}{{/if}}  class="{{$classAttributeTmp3}}">
													<input class="return_type" name="return_type" type="radio" {{if $loop364->getItem("@checked") == (string)'true'}} checked="checked" {{/if}} value="{{$loop364->getItemEscape("@id")}}"></input>
													<img class="return_form" alt="" src="{{$loop364->getItemEscape("@icon")}}"></img>
													<div class="return_form_sub">
														<div class="return_form_name">
															{{if  $loop364->getItem("@id")  == (string) 't'}}
																{{$txt['107849::51900_1']}}
															{{elseif  $loop364->getItem("@id")  == (string) 'b'}}
																{{$txt['107849::51900_2']}}
															{{elseif  $loop364->getItem("@id")  == (string) 'tc'}}
																{{$txt['107849::51900_3']}}
															{{elseif  $loop364->getItem("@id")  == (string) 'cashback'}}
																{{$txt['107849::51900_4']}}
															{{else}}
																{{$loop364->getItemEscape("@name")}}
															{{/if}}
														</div>
														<div class="return_form_desc">
															...
															{{elseif  $loop364->getItem("@id")  == (string) 'cashback'}}
																...
																{{if  $loop364->getItem("@payment_system")}}
																	{{$txt['107849::51900_cashback_payment_system']}}
																	<span>{{$loop364->getItemEscape("@payment_system")}}.</span>
																{{/if}}
															{{else}}
																{{$loop364->getItemEscape("@description")}}
															{{/if}}
														</div>
														{{if  $loop364->getItem("@additional_field_name")}}
															<div class="form-group clearfix">
																<div class="row">
																	<div class="has-feedback has-required col-12">
																		<input type="text" class="return_form form-control validate" minlength="26" data-ajaxtype="get" name="{{$loop364->getItemEscape("@additional_field_name")}}" value="{{$loop364->getItemEscape("@additional_field_value")}}" placeholder="{{$txt['107849::5345234_numformat']}}" {{if $loop364->getItem("@validation") == (string) 'bank_account'}} data-validation="bankAccount" {{else}} data-validation="{{$loop364->getItemEscape("@validation")}}" {{/if}}{{if $loop364->getItem("@validation_url") == (string) '/ajax/returns.php'}} data-validation-url="/ajax/payments.php?action=verify_iban" {{else}} data-validation-url="{{$loop364->getItemEscape("@validation_url")}}" {{/if}}></input>
													...
													{{if $n83521_shipping_title and  $loop364->getItem("@id")  == (string) 't' and   $loop364->getItem("/shop/page/repayment_methods/@shipping_balanced_refund")  == (string) 'true'}}
											...
											{{/foreach}}
										{{else}}
											{{foreach from=$xml->getList("page/repayment_methods/repayment_method") item=loop456}}
												<div class="return_form_one">
													<input class="return_type" name="return_type" type="radio" checked="checked" value="{{$loop456->getItemEscape("@id")}}"></input>
													<img class="return_form_one" alt="" src="{{$loop456->getItemEscape("@icon")}}"></img>
													<div class="return_form_one_sub">
														<div class="return_form_one_name">
															{{if  $loop456->getItem("@id")  == (string) 't'}}
																{{$txt['107849::51900_1']}}
															{{elseif  $loop456->getItem("@id")  == (string) 'b'}}
																{{$txt['107849::51900_2']}}
															{{elseif  $loop456->getItem("@id")  == (string) 'tc'}}
																{{$txt['107849::51900_3']}}
															{{elseif  $loop456->getItem("@id")  == (string) 'cashback'}}
																{{$txt['107849::51900_4']}}
															{{else}}
																{{$loop456->getItemEscape("@name")}}
															{{/if}}
														</div>
														<div class="return_form_one_desc">
															...
															{{elseif  $loop456->getItem("@id")  == (string) 'cashback'}}
																{{$txt['107849::51900_cashback']}}
																{{if  $loop456->getItem("@payment_system")}}
																	{{$txt['107849::51900_cashback_payment_system']}}
																	<span>{{$loop456->getItemEscape("@payment_system")}}.</span>
																{{/if}}
															{{else}}
																{{$loop456->getItemEscape("@description")}}
															{{/if}}
														</div>
														{{if  $loop456->getItem("@additional_field_name")}}
															<div class="form-group clearfix">
																<div class="row">
																	<div class="has-feedback has-required col-12">
																		<input type="text" class="return_form form-control validate" name="{{$loop456->getItemEscape("@additional_field_name")}}" value="{{$loop456->getItemEscape("@additional_field_value")}}" data-validation="{{$loop456->getItemEscape("@validation")}}"></input>
																		<span class="form-control-feedback"></span>
																	</div>
																</div>
															</div>
									...
									{{foreach from=$xml->getList("page/repayment_methods/repayment_method[@additional_field_name][1]") item=loop532}}
